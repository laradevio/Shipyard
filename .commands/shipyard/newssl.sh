...
rm -f .secrets/ssl/*index* >> .secrets/ssl/newssl.log 2>&1 && \
...
openssl ca -policy signing_policy -extensions signing_req -crlexts crl_ext -batch \
	...
	-infiles .secrets/ssl/certs/shipyard-server-cert.csr >> .secrets/ssl/newssl.log 2>&1
...
printf "\n" >> .secrets/ssl/newssl.log 2>&1
openssl verify -CAfile .secrets/ssl/certs/shipyard-ca-cert.pem .secrets/ssl/certs/shipyard-server-cert.pem >> .secrets/ssl/newssl.log 2>&1
printf "##########################################\n\n\n" >>.secrets/ssl/newssl.log 2>&1
