...
# Internal Notes on Preinstalled Sofware

*~ This part does not get published officially in the file; Is for discussion only ~*

Some hard decisions had to be made to make Laravel Shipyard a starting point for a development stack without becoming a swiss-knife jack-of-all-trades-and-what-not-and-hard-to-maintain. It leaves the task of adding more Service containers to the user.

Here are some explanations for the left-out software I initally wanted to integrate but I just couldn't bear to justify:

- **Any JAVA thing**: Basically JAVA. Fuck JAVA. Fuck Oracle. 🖕
- **Any Microsoft thing**: [Don't get me started on this](https://www.reddit.com/r/copypasta/comments/2qmqzz/an_essay_to_insult_someone/).🤐
- **Any Message Queues**: RabbitMQ, ActiveMQ, IronMQ, Kafka, are not included. They're good for a message-centric App, but a PHP Worker with Redis can suffice in the meantime. I'll someday test IronMQ if it works OOB, and if it is, add it to the stack, to send mails outside Shipyard async without mhsendmail.
- **Any NoSQL Databases**: Kind of confusing for starting a Laravel application, and they fill other specific purposes. This includes MongoDB, Neo4j, RethinkDB.
- **Any Search Indexing Engines**: 99% works with JVM. Read first list item. Also, Algolia rocks.🤘
- **Any Database Management Apps**: The user is free to use a software like HeidiSQL or include a container with PhpMyAdmin or Adminer under a php:alpine image. Or even MySQL Workbench. 
- **Any FTP / File Explorer tools**: We just bound the Application directory to the Host. Useful on production only, with proper security. Warehouse SFTP can work too.
- **Any Other Warehouse tools**: Just to keep it simple and related only for what is offered.
~
- **Aerospike**: **HAS TELEMETRY ON BY DEFAULT**. Due to that is left out, but is easy to integrate and opt-out nonetheless. Nor Shipyard or the dev should *work* for Aerospike, but the other way around.
- **Apache**: Nginx.
- **Blackfire**: Works better in development... with a proper team and time to spare instead of rushing MVP.
- **Caddy**: Good fire-and-forget HTTPS server but it won't be on almost any real server at deployment unless you go homebrew.
- **Certbot**: Production focused tool.
- **Concourse**: Travis Self-hosted, basically. Not bad, but is the focus on big projects and a responsable team.
- **Gearman**: Beanstalkd.
- **Gitlab**: Pay for Github unless your team needs a free private Git using sweat, patience and time.
- **HAProxy**: Production focused tool.
- **HHVM**: Unless OPCache is not what you need, you have a lot of server resources, and nanoseconds can make a difference, you are free to dive alone in HHVM.
- **Jenkins**: Check the very first bullet point.
- **Mattermost**: Slack self-hosted seems good but is for big teams.
- **Memcache**: Redis is more powerful and with better support. Sorry mem'.
- **Minio**: Almost goes in; It's even referenced on the "Extend Shipyard" part, but a Laravel App not always requires this.
- **Percona**: Enterprise-grade MySQL / MongoDB Server. We are just starting, so the included should suffice.
- **Swoole**: Beanstalkd and Node.Js takes care of the lot. There are [some scenarios](https://github.com/swoole/swoole-src#why-swoole) were you may want Swoole 🤔
- **Varnish**: Production focused tool.
~
- **Docker Sync**: Killed. Shipyard uses `docker-compose.yml` version 3.3 and its not gracefully compatible with it. `:delegated` should suffice.

## FAQ

* Is Shipyard Multi-site?

No. Doing it multi-site will depend on the user. Some may want to emulate four separate servers, others will no have problems in sharing one PHP Service and SSL Certs, there is the Shipyard Speaker replication, etc. While it may be more simple to just copy & paste Shipyard to another location, I will leave it to the user or another fork.

* Why do you use TSL?

All the web is revolving around HTTP2 and secure conections. A lot of services only work if the site, even in local development, is HTTPS, like Google Maps API. Instead of letting the user mounting TSL by their own, Shipard makes it work OOB. Never worry about TLS Certificates again!

Also, Shipyard doesn't come with SSL Certs on Github, it creates them in the computer is installed.

* Why you not run OpenSSL to create certificates inside a container (like Warehouse) instead of asking to install it?

Because Linux & Mac already have OpenSSL (and can be updated), and for `.\shipyard` commands to work in Windows you need Git for Windows, that has the latest OpenSSL and MINGW64. So, unnecesary even to make a self-contain OpenSSL container for this if only Windows users have to install one tiny thing more.

* Why MariaDB/PostgreSQL don't save data on Windows?

Permissions problems. Docker for Windows shares a drive using SMB/CIFS, and is not totally compatible with Linux permissions. So when they start, the engine cannot take proper ownerage of the files. The ball is on Docker court [since last year](https://github.com/docker/for-win/issues/39), and as long it not fixed, there can be only one thing: tell the user that MariaDB and PostgreSQL cannot save data but it can be dumped using any program of their choice. MySQL works fine, nonetheless.

* Why you are not using the super-latest PHP & Node.js?

It's imposed by Alpine's repos. Changes are not THAT big, but mixing packages from the `@edge` and `@stable` just for the sake of bump from 7.1.5 to 7.1.9 sometimes gives errors because the libraries are different. It happened a lot during Shipyard dev and testing, so I went to the *Docker should be easy* route.

* Why `npm install` last so much in my battlestation?

Because it has to install everything into the giant `node_modules` folder, but using Linux-compatible binaries sometimes. After a vanilla Laravel npm proudly said:

    added 1298 packages in 362.397s

What the great fuck? What is this vanilla Laravel Mix? What is life?
